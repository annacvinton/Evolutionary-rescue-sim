...
pdf("fig6_trajectories_v2.pdf", width = 10, height = 5.4)
par(
  mfcol = c(2, 4), mar = c(3.6, 4.2, 2.4, 0.8), mgp = c(2.4, .6, 0),
...
for (k in seq_along(conds)) {
  ...
  plot(NA,
    xlim = c(0, 810), ylim = c(1, 1500), log = "y",
  ...
  if (k == 1) {
    ...
    legend("bottomright", c("individual runs", "median"),
      col = c(adjustcolor(pal[k], .5), pal[k]),
  ...
  mtext(sprintf("%.0f%% persist", 100 * mean(last$n > 10)),
    side = 3,
...
dev.off()
cat("wrote fig6_trajectories_v2.pdf\n")
