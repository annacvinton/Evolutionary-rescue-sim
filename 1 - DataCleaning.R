...
if (length(part2_fs) > 0) {
  for (name in part2_fs) {
    ...
    if (min(unique(data_2$rep)) < 100) {
      data_2$rep <- data_2$rep + 100
    }
    ...
    function(x) {
      ...
      if (length(duplicates) != 0) {
        ...
        for (i in duplicates) {
          ...
          if (is.na(finRow[length(finRow)])) {
            finRow[length(finRow)] <- nrow(data_df)
          }
          ...
          if (is.na(end[length(end)])) {
            end[length(end)] <- nrow(data_df)
          } # this catches when we have to delete the last sim in the file
      ...
      saveRDS(data_df, file = file.path(
        Dir.Data,
