...
Data_ls <- pblapply(Sims_fs,
  ...
  function(x) {
    ...
    if (file.exists(file.path(Dir.Exports, paste0("TEMP_TStepPop_", x, ".RData")))) {
      ...
      ## Trait measures ----
      u_mean <- aggregate(u ~ t + pert.name + rep,
        data = data_df,
        FUN = mean
      )$u
      u_sd <- aggregate(u ~ t + pert.name + rep,
        data = data_df,
        FUN = sd
      )$u
      u_skewness <- aggregate(u ~ t + pert.name + rep,
        data = data_df,
        FUN = e1071::skewness
      )$u
      u_kurtosis <- aggregate(u ~ t + pert.name + rep,
        data = data_df,
        FUN = e1071::kurtosis
      )$u
      ...
      MalAdap_mean <- aggregate(MalAdaptedness ~ t + pert.name + rep,
        data = data_df,
        FUN = mean
      )$MalAdaptedness
      MalAdap_sd <- aggregate(MalAdaptedness ~ t + pert.name + rep,
        data = data_df,
        FUN = sd
      )$MalAdaptedness
      MalAdap_skewness <- aggregate(MalAdaptedness ~ t + pert.name + rep,
        data = data_df,
        FUN = e1071::skewness
      )$MalAdaptedness
      MalAdap_kurtosis <- aggregate(MalAdaptedness ~ t + pert.name + rep,
        data = data_df,
        FUN = e1071::kurtosis
      )$MalAdaptedness
      ...
      NNDist_mean <- aggregate(NN_Distance ~ t + pert.name + rep,
        data = data_df,
        FUN = mean, na.action = NULL
      )$NN_Distance
      NNDist_sd <- aggregate(NN_Distance ~ t + pert.name + rep,
        data = data_df,
        FUN = sd, na.action = NULL
      )$NN_Distance
      NNDist_skewness <- aggregate(NN_Distance ~ t + pert.name + rep,
        data = data_df,
        FUN = e1071::skewness, na.action = NULL
      )$NN_Distance
      NNDist_kurtosis <- aggregate(NN_Distance ~ t + pert.name + rep,
        data = data_df,
        FUN = e1071::kurtosis, na.action = NULL
      )$NN_Distance
