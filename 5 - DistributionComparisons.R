  ...
  Overlap_ls <- pblapply(
    ...
    FUN = function(x) {
      ...
      Iter_ls <- lapply(Iter_fs, FUN = function(y) {
        ...
        Characteristics <- as.numeric(unlist(regmatches(
          ...
          rep_ls <- lapply(unique(EvoResSuc_iter$rep), FUN = function(b) {
            ...
            for (i in 1:nrow(comparison)) {
              ...
              |
                ...
                OV <- overlap(
                  list(
                    X2 = plot_df$Value[plot_df$Origin == "Environment" & plot_df$Time == comparison$Envir[i]],
...
if (file.exists(file.path(Dir.Exports, "DISTRIBUTIONS_Spatial.RData"))) {
  ...
  Overlap_ls <- pblapply(
    ...
    FUN = function(x) {
      ...
      Iter_ls <- lapply(Iter_fs, FUN = function(y) {
        ...
        Pert_ls <- lapply(perts_vec,
          ...
          FUN = function(z) { # [1:2]
            ...
            rep_ls <- lapply(unique(EvoResSuc_iter$rep),
              ...
              FUN = function(b) {
                ...
                if (file.exists(file.path(Dir.Exports, paste0("TEMP_", y, "_", z, "_", b, "_NoEMD.RData")))) {
                    ...
                    FUN = function(comp_iter) {
                      ...
                      if (nrow(Indivs_sp) == 0) { # this should not be triggered anymore
                        ...
                        Best_df <- apply(data.frame(Indivs_sp), MARGIN = 1, FUN = function(p) {
                          ...
                          ## Distance to
                          Best_dist <- pointDistance(
                            p1 = Indiv_iter[c("x", "y")], # individual location,
                        ...
                        Closest_df <- data.frame(
                          Closest_diffs = unlist(lapply(Closest_ls, "[[", "Closest_diffs")),
                  ...
                  Up_ls <- list(
                    Summary_df = do.call(rbind, lapply(comp_ls, "[[", "Summary_df")),
            ...
            Up_ls <- list(
              Summary_df = do.call(rbind, lapply(rep_ls, "[[", "Summary_df")),
        ...
        Up_ls <- list(
          Summary_df = do.call(rbind, lapply(Pert_ls, "[[", "Summary_df")),
      ...
      Up_ls <- list(
        Summary_df = do.call(rbind, lapply(Iter_ls, "[[", "Summary_df")),
        RasterDiff = lapply(Iter_ls, "[[", "RasterDiff")
      )
      Up_ls
    }
  ) # Overlap_ls loop
  DISTRIBUTIONS_Spatial <- list(
    Summary_df = do.call(rbind, lapply(Overlap_ls, "[[", "Summary_df")),
...
if ("EMD2D" %nin% colnames(DISTRIBUTIONS_Spatial$Summary_df)) {
  ...
  EMD_df <- do.call(rbind, Overlap_ls)
  DISTRIBUTIONS_Spatial$Summary_df <- base::merge.data.frame(DISTRIBUTIONS_Spatial$Summary_df, EMD_df)
  save(DISTRIBUTIONS_Spatial, file = file.path(Dir.Exports, "DISTRIBUTIONS_Spatial.RData"))
  unlink(list.files(Dir.Exports, pattern = "TEMP_", full.names = TRUE))
  stopCluster(cl)
}
